...
class FlowTranspiler(Transformer):
    ...
    def agg_function(self, a):
        func_name = a[0]
        col_name = a[1] if len(a) > 1 else None
    ...
    def source_decl(self, s):
        ...
        if schema_name_list:
            schema_name = schema_name_list[0] # <-- FIX was here
        ...
        if func_call['name'] == 'File':
            ...
            user = args['user'][1:-1]
            host = args['host'][1:-1]
            database = args['database'][1:-1]
            table = args['table'][1:-1]
    ...
    def pipeline(self, p):
        ...
        start_py_var = self.variables.get(start_flow_var) # <-- FIX was here
        ...
        # This part of the logic now depends on having self.group_by_cols available from the validator.
        # This is a bit of a design flaw, but we'll proceed for now. A better design would pass state explicitly.
        self.group_by_cols = [] 
        ...
        for item in steps:
            ...
            if isinstance(item, tuple):
                ...
                if op_type == 'filter':
                    code.append(f"{next_py_var} = {current_py_var}[{op_args.replace('{df}', current_py_var)}]")
                elif op_type == 'select':
                    ...
                    self.group_by_cols = op_args # Store for aggregate step
                    ...
                    for new_col, (src_col, func) in op_args.items():
                        # Use the first group_by column for a column-less count()
